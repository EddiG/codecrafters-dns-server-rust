...
mod tests {
    ...
    fn valid_question_and_answer_response_message() {
        ...
        let bytes = message.to_be_bytes();
        insta::assert_snapshot!(format!("{:02x?}", &bytes), @"[00, 01, 80, 00, 00, 01, 00, 01, 00, 00, 00, 00, 06, 67, 6f, 6f, 67, 6c, 65, 03, 63, 6f, 6d, 00, 00, 05, 00, 01, 06, 67, 6f, 6f, 67, 6c, 65, 03, 63, 6f, 6d, 00, 00, 05, 00, 01, 00, 00, 0e, 10, 00, 04, 08, 08, 08, 08]");
    }
}
